...
if (process.argv.includes('-h') || process.argv.includes('--help')) {
  console.log(`
Usage: node HTTP-IOS <host> <duration> <rate(8)> [options]
Credits: @h2socket / @udpboy | HTTP-IOS
...
Example:
  node HTTP-IOS http://captcha.count123.org 120 4 --proxy proxies.txt --headers undetect
  `);
  process.exit(0);
}

if (process.argv.length < 5) {
    console.error("Usage: node HTTP-IOS <host> <duration> <rates> [options]\nnode HTTP-IOS -h for help");
    process.exit(1);
}
